% The double-stranded lattice sum by frontier elimination   (Bio 12 / XXV)
% Build: pdflatex anccft25 (x3)
...
\title{\bfseries The double-stranded lattice sum\\ by frontier elimination}
...
\begin{abstract}
\noindent
Bio~8 wrote the number of double-stranded reconstructions of a circular sequence as a sum over
...
row of that note's Table~2. The obstruction is not the rank. The circulation condition is one
...
$909\,103\,210$; both columns Bio~8 left empty are now filled, and only $\#\DS$ itself is not.
Two further things follow. Connectivity of $\supp c$,
carried in Bio~8 as a separate filter, is not a side condition at all: a balanced digraph has
...
$26$ wide, which is the interleaving of inverted repeats of Bio~1 seen as a width.
\end{abstract}

\medskip
\noindent\textbf{Keywords.} de Bruijn graph, reverse-complement double cover, circulation
lattice, lattice point counting, frontier elimination, transfer matrix, pathwidth,
matrix-tree theorem, genome assembly.

\section{What is being counted}\label{sec:setup}

We keep the notation of \cite{Bio8} exactly. $S$ is a circular sequence of length $N$,
$\bar S$ its reverse complement, and $D_k=D_k(S)$ the labelled reverse-complement double
cover: its vertices are the $k$-mers occurring at least twice in the two-strand pool, its
labelled arcs the maximal stretches between consecutive such $k$-mers along each strand, and
two arcs carry the same \emph{content} when they spell the same word. Write $A$ for the set of
contents, $m(a)$ for the number of labelled arcs of content $a$, and $\rho$ for the
reverse-complement involution, $\rho a=\overline a$; then $m(\rho a)=m(a)$, and $D_k$ is
balanced.

A circular sequence $T$ is a \emph{double-stranded reconstruction} of $S$ at word length $k$
when $\spec(T)+\spec(\bar T)=\spec(S)+\spec(\bar S)$. With $c=\spec(T)\in\Z_{\ge0}^A$ this says
$c+\rho c=m$ and $c$ is a circulation; with $f=2c-m$ it says $f$ lies in the $(-1)$-eigenlattice
$\Lm=\{f\in Z_1(D_k):\rho f=-f\}$, in the box $|f|\le m$, in the parity class of $m$. Hence
\cite[Thm.~7]{Bio8}
...
\end{equation}
the sum over $f\in\Lm$ in the box, where $G_c$ is the multidigraph with $c(a)$ copies of each
content, $t(G_c)$ its number of spanning arborescences to a fixed root, and
$\Loc(G_c)=\prod_v\big(d^+_c(v)-1\big)!$. The division by $\prod_a c(a)!$ passes from labelled
arcs to sequences.

Bio~8 enumerated the box by backtracking over a basis of $\Lm$ with capacity pruning, and
...
\section{The constraint system and its frontier}\label{sec:frontier}

The point of this note is that \eqref{eq:sum} is indexed by the solutions of a very sparse
linear system, and that sparsity, not the rank, is what governs the count.

\begin{definition}[the orbit variables]\label{def:vars}
...
\begin{definition}[frontier]\label{def:front}
...
\begin{theorem}[frontier elimination]\label{thm:elim}
...
satisfying \eqref{eq:bal}. Process the orbits in the order $\pi$ and record, after step $i$,
the partial sums $x(v)=\sum_{\ell\le i}B(v,j_\ell)g_{j_\ell}$ at the open vertices. A vertex
$v$ that is not open after step $i$ receives no further contribution, so \eqref{eq:bal} at $v$
...
\begin{remark}[what the cost does and does not depend on]\label{rem:cost}
Theorem~\ref{thm:elim} contains no reference to $\rank\Lm$. The backtracking of \cite{Bio8}
...
\begin{proposition}[product-form weights]\label{prop:weights}
Let $u_j$ be functions on the values of $g_j$ and $h_v$ functions on $\Z_{\ge0}$, and
\[
w(c)=\prod_j u_j(g_j)\prod_v h_v\big(d^+_c(v)\big).
\]
Then $\sum_f w(c_f)$ over the box is computed by Theorem~\ref{thm:elim} after enlarging the
...
\section{Connectivity is the matrix-tree determinant}\label{sec:conn}

Bio~8 carried, beside the box conditions, a predicate: the support of $c$ must be connected,
or the point contributes nothing. The predicate is real but it is not an extra condition.

\begin{lemma}[connectivity is not a side condition]\label{lem:conn}
...
may be deleted from \eqref{eq:sum} without changing the sum.
\end{lemma}
...
Lemma~\ref{lem:conn} removes is the impression that \eqref{eq:sum} is a lattice sum
\emph{subject to a combinatorial constraint}. It is a lattice sum, full stop; the constraint
is already inside $\Nseq$. This is also the answer, in the only form it has, to the question of
\cite{Bio8} for an algebraic criterion of connected support avoiding graph traversal: the
...
\begin{theorem}[connected points]\label{thm:connected}
Enlarge the state of Theorem~\ref{thm:elim} by (i) the partition of $\Fr_i$ into the classes
...
\begin{remark}[the connectivity state is cheaper than it looks]\label{rem:bell}
The Bell factor in Theorem~\ref{thm:connected} is a gross overestimate, because the rule
...
\section{The genome's own coordinate is not the good order}\label{sec:width}

Theorem~\ref{thm:elim} turns the problem into the choice of $\pi$. There is an obvious
...
\begin{proposition}[the coordinate order]\label{prop:coord}
...
\begin{proof}
Immediate from Definition~\ref{def:front}: the vertices met by orbit $j$ are endpoints of
$a_j$ and $\rho a_j$, and a vertex is open only between the first and last orbit meeting it.
\end{proof}

The hypothesis is the point. Table~\ref{tab:width} gives the widths.
...
\begin{remark}[why the coordinate order fails]\label{rem:interleave}
...
sequence \emph{interleave} --- that is the content of the interleaving theorem of \cite{Bio1},
...
\section{$\phi$X174 at $k=10$}\label{sec:phix}
...
\bottomrule
\end{tabular}
\caption{The blank row of \cite[Table 2]{Bio8}, filled in both of its counting columns. The
...
reachable by enumeration: $2.48\times10^9$ points at the rate at which \cite{Bio8} enumerates
...
\begin{remark}[the connected fraction]\label{rem:frac}
...
\begin{remark}[what is still missing, exactly]\label{rem:wall}
Of the three counting columns of Table~\ref{tab:phix} only $\#\DS$ is now empty at $k=10$, and
it is empty for a reason of a different kind from the other two. Those were questions of state
size: Theorem~\ref{thm:elim} settles the first in seconds and Theorem~\ref{thm:connected} the
second in an hour and a quarter.

The $\#\DS$ column is a question of principle. By
Proposition~\ref{prop:weights} the elimination sums any weight that factors into per-orbit and
...
determinants of order up to $125$. So $\#\DS(\phi X174,10)$ remains out of reach, and it is
now clear that what stands in the way is the arborescence factor and not the lattice --- the
index set is known exactly, and it is the summand that cannot be carried. Bio~8's Table~2
...
\texttt{numpy}, no \texttt{sympy} and no other file of the series --- and rebuilds $D_k$ from
the definition rather than importing it, so that check (D) is a genuine cross-check of Bio~8's
Table~1 and not a tautology. Exact integer arithmetic throughout; the determinants of
Lemma~\ref{lem:conn} are computed over $\mathbb Q$ with \texttt{fractions}.
...
$|A|$, $\#\mathrm{fix}_A$, $\sum m$ and $\rank\Lm$ of \cite[Table 1]{Bio8} at $k=10,11,12$\\
...
each of two orbits\\
(N) & the connectivity-class elimination returns Bio~8's $8$ and $218$\\
...
\section{Scope, provenance, and what is open}\label{sec:scope}

\paragraph{Scope.} Theorem~\ref{thm:elim}, Proposition~\ref{prop:weights},
Lemma~\ref{lem:conn} and Theorem~\ref{thm:connected} hold for any circular $S$ and any $k$.
Proposition~\ref{prop:coord} is a bound, and Table~\ref{tab:width} shows it is not a good one
on real sequence. Section~\ref{sec:phix} is a computation on one named genome and claims
nothing beyond it. Nothing here improves \eqref{eq:sum} itself, which is Bio~8's.

\paragraph{Provenance.} The lattice sum, $\Lm$, its rank by the Hopf trace formula, and the
$k=12$ and $k=11$ values are \cite{Bio8}; the double cover and its balance are \cite{Bio6},
its deck involution as an anti-automorphism \cite{Bio7}, the interleaving of repeats
\cite{Bio1}. The BEST theorem is \cite{AEdB,SmithTutte} and the matrix-tree theorem for
...
Theorem~\ref{thm:connected} --- carrying a partition of the frontier so as to count connected
...
$t(G_c)$; the natural attempt is to carry a partial Schur complement of $L_c$ on the frontier,
which is a matrix state and not a scalar one, and whether that can be made to fit is the real
question this note leaves. An order chosen to keep partitions few rather than imbalances small
would help both it and Theorem~\ref{thm:connected}, whose cost at $k=10$ is entirely the
...
whether the coordinate width of Proposition~\ref{prop:coord} is, for a random sequence with a
...
\bibitem{AEdB} T.~van Aardenne-Ehrenfest and N.~G.~de Bruijn, \emph{Circuits and trees in
oriented linear graphs}, Simon Stevin \textbf{28} (1951), 203--217.

\bibitem{Barvinok94} A.~I.~Barvinok, \emph{A polynomial time algorithm for counting integral
points in polyhedra when the dimension is fixed}, Math.\ Oper.\ Res.\ \textbf{19} (1994),
769--779.

\bibitem{Bodlaender98} H.~L.~Bodlaender, \emph{A partial $k$-arboretum of graphs with bounded
treewidth}, Theoret.\ Comput.\ Sci.\ \textbf{209} (1998), 1--45.

\bibitem{Bio1} R.~Chen, \emph{The sandpile group of a de Bruijn graph and the interleaving of
repeats}, companion note (Bio 1), September 2026.

\bibitem{Bio6} R.~Chen, \emph{Double strands without a new lemma: the reverse-complement
double cover}, companion note (Bio 6), September 2026.

\bibitem{Bio7} R.~Chen, \emph{The reverse-complement quotient is a signed graph}, companion
note (Bio 7), September 2026.

\bibitem{Bio8} R.~Chen, \emph{Double-stranded genome assembly as a lattice sum, and the
inversion-flip group}, companion note (Bio 8), September 2026.

\bibitem{KIIM17} J.~Kawahara, T.~Inoue, H.~Iwashita and S.~Minato, \emph{Frontier-based search
for enumerating all constrained subgraphs with compressed representation}, IEICE Trans.\
Fundamentals \textbf{E100-A} (2017), 1773--1784.

\bibitem{Knuth4A} D.~E.~Knuth, \emph{The Art of Computer Programming, Volume 4A:
Combinatorial Algorithms, Part 1}, Addison-Wesley, 2011, \S7.1.4.

\bibitem{SmithTutte} C.~A.~B.~Smith and W.~T.~Tutte, \emph{On unicursal paths in a network of
degree four}, Amer.\ Math.\ Monthly \textbf{48} (1941), 233--237.

\bibitem{Tutte48} W.~T.~Tutte, \emph{The dissection of equilateral triangles into equilateral
