% Polyploid phasing as a p-fold decomposition of a balanced flow, and the homogeneous space of alleles   (Bio 9 / XXII)
% Build: pdflatex anccft22 (x3)
...
\title{\bfseries Polyploid phasing as a $p$-fold decomposition of a balanced flow,\\
and the homogeneous space of alleles}
...
The preceding note \cite{Bio8} showed that a double-stranded reconstruction of a circular
...
the pool is the disjoint union and there is nothing to phase (Proposition~\ref{prop:cover}).
...
$\prod_j\binom{p}{a_j}$ (Theorem~\ref{thm:sites}), and $(p!)^b$ is wrong already at $p=3$,
$b=1$, $a=1$: the answer is $3$, not $6$.

What survives, and is the subject of Section~\ref{sec:lattice}, is the lattice sum. The
...
\]
with $\Nseq$ the per-sequence BEST count of \cite{Bio8}; the unlabelled phasings by a multiset
formula over the unordered decompositions (Section~\ref{sec:unlabelled}); and when the parts
are not required to have the reference length the sum exposes the copy-number ambiguity
through repeats, including tandem-dimer circles whose weight is $1/2$ (Section~\ref{sec:cnv}).
Section~\ref{sec:data} records the verification on $\phi$X174.
...
\section{The pooled multigraph and the phasing lattice}\label{sec:lattice}

\begin{definition}\label{def:pool}
...
The compaction criterion differs from the one used for a single genome in \cite{Bio5}, where a
...
Definition~\ref{def:pool} contracts exactly the $k$-mers through which every haplotype must
pass in the same way. The per-sequence count below is intrinsic and does not depend on which
compaction is used.

\begin{definition}\label{def:nseq}
...
\]
the second equality being \cite[Thm.~6]{Bio8}. $\Nseq(c)=0$ if the support is disconnected.
...
\begin{definition}\label{def:phasing}
...
\begin{theorem}[The phasing lattice]\label{thm:lattice}
...
\begin{proof}
Lemma~\ref{lem:spectrum} sends each $T_i$ to a balanced connected $c_i\ge0$, and
...
$\Nseq(c_i)$ by Definition~\ref{def:nseq}. Lengths are preserved by Lemma~\ref{lem:spectrum}.
\end{proof}

\begin{remark}
The lattice is the same object as in \cite{Bio8}: there $p=2$, $c_2=\rho c_1$ is forced, and
...
\begin{proposition}[No covering]\label{prop:cover}
...
locally by homogeneous spaces (Section~\ref{sec:sites}) and globally by a multiset count
(Section~\ref{sec:unlabelled}).

\section{The homogeneous fibre, and linkage}\label{sec:sites}
...
\begin{theorem}[Independent sites on a repeat-free reference]\label{thm:sites}
...
homogeneous spaces stated, of cardinalities the multinomial coefficients. Theorem~\ref{thm:lattice}
gives the count.
\end{proof}

\begin{corollary}\label{cor:notH1}
Under the hypotheses of Theorem~\ref{thm:sites} the pooled graph has $\pi_1$ free of rank
...
\begin{theorem}[Linkage]\label{thm:linkage}
Let a set $B$ of sites lie in one $(k{+}1)$-window, with the hypotheses of
Theorem~\ref{thm:sites} otherwise in force for $B$ as a whole (no $k$-mer overlapping $B$ is a
...
Theorem~\ref{thm:sites} the block $B$ counts as one site whose alleles are its local haplotypes.
...
\begin{remark}[With repeats]\label{rem:repeats}
When $R$ has repeats the argument of Theorem~\ref{thm:sites} gives a lower bound. Each allele
...
every tested configuration (Table~\ref{tab:diploid}).
\end{remark}

\section{The unlabelled multiset formula and the
\texorpdfstring{$\Sym_p$}{Sp}-action}\label{sec:unlabelled}

\begin{proposition}\label{prop:multiset}
...
\begin{remark}[Periodic parts]\label{rem:periodic}
...
case $\Nseq(c)$ is a half-integer or worse. The weighted count of Theorem~\ref{thm:lattice}
...
they are the dimer circles of Section~\ref{sec:cnv}.
\end{remark}

\section{Copy-number variation across repeats}\label{sec:cnv}

The equal-length constraint encodes the substitution-only model. Without it, the sum of
Theorem~\ref{thm:lattice} ranges over all decompositions of the pooled flow into closed walks,
...
\cite{Bio1}, $a\,X_1\,b\,Y_1\,a\,X_2\,b\,Y_2$: two branch $12$-mers $a,b$ and four unitigs,
...
(Table~\ref{tab:cnv}). A pooled spectrum thus does not distinguish two identical circular
...
Tables~\ref{tab:diploid}--\ref{tab:cnv} is computed twice: from the lattice, by enumerating the
...
\paragraph{Scope.} Theorem~\ref{thm:lattice}, Proposition~\ref{prop:cover} and
Proposition~\ref{prop:multiset} hold for arbitrary circular haplotypes. Theorem~\ref{thm:sites},
Corollary~\ref{cor:notH1} and Theorem~\ref{thm:linkage} assume a repeat-free reference and
isolated sites; Remark~\ref{rem:repeats} is a lower bound for references with repeats, met with
...
\paragraph{Provenance.} The BEST theorem is \cite{AEdB,SmithTutte}, its per-sequence
normalisation and the notation $\Nseq$ are \cite{Bio8}, and the single-genome decomposition
of $\phi$X174 at $k=12$ is \cite{Bio1}. Haplotype assembly as a combinatorial problem on
...
Remark~\ref{rem:repeats}: characterise the references for which $p\,\spec(R)$ has no
...
is the natural tool, as it is for the double-stranded sum of \cite{Bio8}. \emph{Fourth}, the
...
\bibitem{AI12} D.~Aguiar and S.~Istrail,
\emph{HapCompass: a fast cycle basis algorithm for accurate haplotype assembly of sequence
data}, J. Comput. Biol. \textbf{19} (2012), no.~6, 577--590.

\bibitem{BB08} V.~Bansal and V.~Bafna,
\emph{HapCUT: an efficient and accurate algorithm for the haplotype assembly problem},
Bioinformatics \textbf{24} (2008), no.~16, i153--i159.
...
\emph{Escherichia coli} K-12}, companion note (Bio 5), September 2026.
...
\bibitem{Garg21} S.~Garg,
\emph{Computational methods for chromosome-scale haplotype reconstruction},
Genome Biology \textbf{22} (2021), 101.
...
\bibitem{LBIL01} G.~Lancia, V.~Bafna, S.~Istrail, R.~Lippert and R.~Schwartz,
\emph{SNPs problems, complexity, and algorithms},
in: Algorithms --- ESA 2001, Lecture Notes in Computer Science \textbf{2161}, Springer, 2001,
pp.~182--193.
