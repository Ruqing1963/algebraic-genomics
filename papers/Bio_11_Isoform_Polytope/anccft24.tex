% The isoform decomposition polytope of a splicing graph, and a BEST identity for its integer points   (Bio 11 / XXIV)
% Build: pdflatex anccft24 (x3)
...
\title{\bfseries The isoform decomposition polytope of a splicing graph,\\
and a BEST identity for its integer points}
...
graph \cite{Bio1}, double-stranded ambiguity off the box points of an eigenlattice of the
strand involution \cite{Bio8}, and polyploid phasing off the $p$-fold decompositions of a
pooled flow \cite{Bio9}. The common object is the decomposition of a balanced integer flow
...
of them (Proposition~\ref{prop:vertex}). But the path--arc matrix is not totally unimodular,
and $\Th(w)$ has fractional vertices for integer $w$ (Proposition~\ref{prop:frac}); the
...
What is true and, we believe, new is in Sections~\ref{sec:dim} and~\ref{sec:best}. The
dimension of $\Th(w)$ is exactly the number of paths in the support of $w$ minus the rank of
the flow space of that support (Theorem~\ref{thm:dim}); this is the precise form of the
...
$\Th(w)$ as a determinant (Theorem~\ref{thm:best}): the same normalisation by
$\prod c(a)!$ that \cite{Bio8} needed for double strands appears here as the passage from
...
\section{The splicing graph, its flows, and the fibre polytope}\label{sec:dim}
...
\begin{theorem}[Dimension of the isoform polytope]\label{thm:dim}
...
lies on a path of positive flow, by flow decomposition), so Lemma~\ref{lem:rank} gives
...
\begin{corollary}[Events in series]\label{cor:series}
...
lengths. The last claim is Theorem~\ref{thm:dim}.
\end{proof}

The identifiability statement in Theorem~\ref{thm:dim} is the rank condition of
\cite{HJXW09} specialised to junction-and-exon counts; the dimension formula makes it
quantitative, and Corollary~\ref{cor:series} says that for any gene with two independent
alternative events the counts do not determine the abundances, whatever the depth of
sequencing. Table~\ref{tab:models} lists the models and their dimensions.

\section{Vertices, minimum decompositions, and integrality}\label{sec:vertices}
...
\begin{proposition}[A fractional vertex]\label{prop:frac}
...
Proposition~\ref{prop:vertex}, and it is not integral. The $3\times3$ submatrix of the
...
\section{The BEST identity}\label{sec:best}
...
connected, so it has Eulerian circuits, and the per-sequence count of \cite{Bio8} applies:
\[
...
\begin{theorem}[BEST identity for isoform decompositions]\label{thm:best}
\[
...
linear orders of $F$ transcripts with those abundances. Theorem~\ref{thm:best} is thus a
...
determinantal formula, and Theorem~\ref{thm:best} says precisely which weighting is
determinantal.
...
with the point: for the tropomyosin-like flow of Table~\ref{tab:flows} the arborescence count
...
fractional vertex of Proposition~\ref{prop:frac} exists; (B) the BEST identity on the four
flows of Table~\ref{tab:flows} and on $232$ random flows on DAGs with $4$ to $6$ vertices;
...
\paragraph{Scope.} Lemma~\ref{lem:rank}, Theorem~\ref{thm:dim},
Corollary~\ref{cor:series}, Proposition~\ref{prop:vertex} and Theorem~\ref{thm:best} are
proved for arbitrary splicing graphs; Proposition~\ref{prop:frac} is one graph. The model is
...
isoform count is \cite{SCH00}; the BEST theorem is \cite{AEdB,SmithTutte} and its
per-sequence normalisation is \cite{Bio8}. The tropical geometry of statistical models,
...
Theorem~\ref{thm:best}; a targeted search found neither.
...
Theorem~\ref{thm:best}. \emph{Third}, the effect of multi-junction reads on the dimension:
...
\emph{Fourth}, the analogue of Theorem~\ref{thm:best} with the return arcs weighted, which
...
\bibitem{Bio1} R.~Chen, \emph{The sandpile group of a de Bruijn graph and the ambiguity of
genome assembly}, companion note (Bio 1), September 2026.

\bibitem{Bio8} R.~Chen, \emph{Double-stranded genome assembly as a lattice sum, and the
inversion-flip group}, companion note (Bio 8, XXI), September 2026.
...
\bibitem{DT22} F.~H.~C.~Dias, L.~Williams, B.~Mumey and A.~I.~Tomescu,
\emph{Fast, flexible, and exact minimum flow decompositions via ILP},
in: Research in Computational Molecular Biology (RECOMB 2022), Lecture Notes in Computer
Science \textbf{13278}, Springer, 2022, pp.~230--245.

\bibitem{HHKS12} T.~Hartman, A.~Hassidim, H.~Kaplan, D.~Raz and M.~Segalov,
\emph{How to split a flow?},
in: Proc.\ IEEE INFOCOM 2012, pp.~828--836.

\bibitem{HAST02} S.~Heber, M.~Alekseyev, S.-H.~Sze, H.~Tang and P.~A.~Pevzner,
\emph{Splicing graphs and EST assembly problem},
Bioinformatics \textbf{18} (2002), suppl.~1, S181--S188.

\bibitem{HJXW09} D.~Hiller, H.~Jiang, W.~Xu and W.~H.~Wong,
\emph{Identifiability of isoform deconvolution from junction arrays and RNA-Seq},
Bioinformatics \textbf{25} (2009), no.~23, 3056--3059.

\bibitem{KKO18} K.~Kloster, P.~Kuinke, M.~P.~O'Brien, F.~Reidl, F.~S.~Villaamil,
B.~D.~Sullivan and A.~van der Poel,
\emph{A practical fpt algorithm for flow decomposition and transcript assembly},
in: Proc.\ Workshop on Algorithm Engineering and Experiments (ALENEX 2018), SIAM, 2018,
pp.~75--86.

\bibitem{LSGB08} V.~Lacroix, M.~Sammeth, R.~Guig\'o and A.~Bergeron,
\emph{Exact transcriptome reconstruction from short sequence reads},
in: Algorithms in Bioinformatics (WABI 2008), Lecture Notes in Computer Science
\textbf{5251}, Springer, 2008, pp.~50--63.

\bibitem{PS04} L.~Pachter and B.~Sturmfels,
\emph{Tropical geometry of statistical models},
Proc.\ Natl.\ Acad.\ Sci.\ USA \textbf{101} (2004), no.~46, 16132--16137.

\bibitem{SCH00} D.~Schmucker, J.~C.~Clemens, H.~Shu, C.~A.~Worby, J.~Xiao, M.~Muda,
J.~E.~Dixon and S.~L.~Zipursky,
\emph{Drosophila Dscam is an axon guidance receptor exhibiting extraordinary molecular
diversity}, Cell \textbf{101} (2000), no.~6, 671--684.
...
\bibitem{VCCM08} B.~Vatinlen, F.~Chauvet, P.~Chr\'etienne and P.~Mahey,
\emph{Simple bounds and greedy algorithms for decomposing a flow into a minimal set of
paths}, European J. Oper.\ Res.\ \textbf{185} (2008), no.~3, 1390--1401.
