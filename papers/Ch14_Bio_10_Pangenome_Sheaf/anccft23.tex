% The pan-genome sheaf sees nothing: the recombination group of a pan-genome graph and its torsion   (Bio 10 / XXIII)
% Build: pdflatex anccft23 (x3)
...
\title{\bfseries The pan-genome sheaf sees nothing:\\
the recombination group of a pan-genome graph, and its torsion}
...
\emph{recombination group} $\Rec=Z_1(G)/\sum_iZ_1(G_i)$, the circulations of the pooled
graph modulo those that live within single strains: the cycles that exist only by switching
strain. The Mayer--Vietoris spectral sequence of the cover $\{G_i\}$ identifies $\Rec$ with the
first homology of the nerve of intersection components, $C_q=\bigoplus_{|\sigma|=q+1}
H_0(G_\sigma)$. For two strains $\Rec$ is free of rank $c(G_1\cap G_2)-1$ --- the number of
...
strains $\Rec$ can have torsion: six synthetic strains whose intersection pattern is the
six-vertex triangulation of the projective plane have $\Rec=\Z/2$, while the octahedron gives
...
$\phi$X174 isolates from GenBank, strand- and origin-normalised, give $\Rec=\Z^{38}$ with all
...
reach but the nerve is not --- $\Rec$ is free, and freely so at every prime at once: the
...
$\Rec=\Z^{117\,848}$ on a nerve with $1.7$ million two-cells, and does so even where $d_2$ has
a large kernel; the rank of $\Rec$ saturates as strains are added, measuring the diversity of
...
graph carries naturally is trivial in a precise sense (Proposition~\ref{prop:sheaf}); but
...
multigraph $G=G_k(H_1{+}\dots{+}H_p)$ of \cite{Bio9} with the decomposition
...
Proposition~\ref{prop:sheaf} says both decompose strain by strain: $\sheaf$ is the direct sum
...
\[
  \Rec(H_1,\dots,H_p;k)=Z_1(G)\Big/\sum_{i=1}^pZ_1(G_i)
\]
...
shared $k$-mer, which is to say the recombination cycles. Section~\ref{sec:R} computes it.
The Mayer--Vietoris spectral sequence of the cover of $G$ by the $G_i$ identifies $\Rec$ with
the first homology of a small chain complex built from the connected components of the
intersections $G_{i_0}\cap\dots\cap G_{i_q}$ (Theorem~\ref{thm:mv}). Two consequences
follow. For two strains, $\Rec$ is free of rank one less than the number of shared blocks
(Corollary~\ref{cor:two}). For three or more, $\Rec$ can have torsion, and
Proposition~\ref{prop:rp2} exhibits six strains with $\Rec=\Z/2$. Section~\ref{sec:data}
verifies all of it, and computes $\Rec$ for six deposited $\phi$X174 genomes.

Throughout, strains are circular sequences over $\{A,C,G,T\}$, $k$ is fixed, and the pooled
graph is unitig-compacted with the branch criterion of \cite{Bio9} (a $k$-mer is a vertex if
...
\section{The pan-genome sheaf}\label{sec:sheaf}
...
\begin{proposition}\label{prop:sheaf}
...
\section{The recombination group}\label{sec:R}

\begin{definition}
$\Rec=Z_1(G)/\sum_iZ_1(G_i)$, where $Z_1(G_i)=Z_1(G)\cap\Z^{A_i}$ is the circulation
...
Mayer--Vietoris spectral sequence of the cover \cite[VII.4]{Brown} applies:
...
\begin{corollary}[Two strains]\label{cor:two}
For $p=2$ with $G_1,G_2$ connected and $G_1\cap G_2\ne\emptyset$, $\Rec$ is free of rank
...
shared subgraph is the $b$ stretches between the bubbles and $\Rec\cong\Z^{b-1}$. When the
...
$\Rec=\Z^2$, three substitutions leave two and $\Rec=\Z$ (Table~\ref{tab:two}). A repeat
...
\begin{proposition}[Torsion]\label{prop:rp2}
...
of components is the simplicial chain complex of $T$, and $\Rec\cong H_1(T;\Z)$. In
particular the six-vertex triangulation of the projective plane gives $\Rec=\Z/2$, the
...
exactly two strains. So $C_\bullet$ is the chain complex of $T$ and Theorem~\ref{thm:mv}
...
decomposes. No pair of strains detects it, because for pairs $\Rec$ is free
(Corollary~\ref{cor:two}); no strain detects it, because within a strain there is nothing to
detect. It is an invariant of the pan-genome as a whole.

\section{Verification and data}\label{sec:data}
...
$\Z^A/\sum Z_1(G_i)$ equals that of $\Rec$ because $Z_1(G)$ is saturated in $\Z^A$), and as
...
\toprule
pool ($k=12$) & $|V|$ & $|A|$ & $\rank Z_1(G)$ & $\rank\sum Z_1(G_i)$ & $c(G_1\cap G_2)$ & $\Rec$\\
\midrule
...
\bottomrule
\end{tabular}
\caption{Two strains. $\Rec$ is free of rank $c(G_1\cap G_2)-1$ in every row; on the
...
\begin{tabular}{lrrrrrl}
\toprule
surface pool ($k=16$) & strains & $|V|$ & $|A|$ & $\rank Z_1(G)$ & $\rank\sum Z_1(G_i)$ & $\Rec=H_1(\Nerve)$\\
\midrule
...
\bottomrule
\end{tabular}
\caption{Proposition~\ref{prop:rp2}. In each pool the nerve of components is verified to be
...
$\rank Z_1(G)=55$, $\rank\sum Z_1(G_i)=17$, $\Rec=\Z^{38}$, no torsion; all $63$
...
the direct sum of the strains' $\Z\oplus\K(|G_i|)$; (B) two strains, eight pools, $\Rec$ free
...
for Section~\ref{sec:real}: (A) the hashed branch criterion against brute force on random
...
\section{A real pan-genome}\label{sec:real}

Theorem~\ref{thm:mv} replaces a lattice quotient by the homology of the nerve, and the nerve
...
$\Rec$ itself, as a quotient of a cycle lattice of rank $10^4$ or more, is not. This section
carries that out.
...
the criterion of \cite{Bio9}: a $k$-mer is a vertex when it has two distinct successors or
...
\begin{proposition}[freeness certificate]\label{prop:cert}
...
summand of $C_1$, and $\Rec=\ker d_1/\im d_2$ is free.
\end{proposition}
...
$\Z$-module is free, and $\Rec=\ker d_1/\im d_2\subseteq C_1/\im d_2$.
\end{proof}

\begin{remark}[a torsion detector without a Smith form]\label{rem:detector}
...
$\dim_{\mathbb F_p}H_1(\Nerve;\mathbb F_p)=\rank\Rec+t_p(\Rec)$ with $t_p$ the $p$-torsion
...
Table~\ref{tab:surfaces}, where the answer is known exactly, and it does return the $\Z/2$ of
...
\toprule
strains ($k=31$) & branch vtx & contents & $C_1$ & $C_2$ & $\rank d_2$ & $\Rec$ & certified\\
\midrule
...
of $d_2$ completes, so by Proposition~\ref{prop:cert} $\Rec$ is free at every prime, and the
rank, being $\dim C_1-\rank d_1-\rank d_2$ with both ranks computed by unimodular integer
elimination, is exact. The two K-12 strains share $86$ blocks and give $\rank\Rec=86-1$,
which is Corollary~\ref{cor:two} on real data. From the fourth strain on $d_2$ is far from
...
\paragraph{The answer.} $\Rec$ is free, with a certificate, on every pool from two to eight
complete chromosomes (Table~\ref{tab:real}). This settles the first open question of an
...
Proposition~\ref{prop:rp2} has exactly one component per simplex, so its nerve \emph{is} a
...
Section~\ref{sec:R} was necessary precisely because real data does not produce that shape,
and the honest reading of Proposition~\ref{prop:rp2} is that torsion is realisable in
principle rather than observed in \emph{E.\ coli}.

\paragraph{The rank saturates.} One thing in Table~\ref{tab:real} is not about torsion. The
successive increments of $\rank\Rec$ are $29\,547$, $36\,815$, $21\,411$, $16\,637$,
...
an observation on these eight genomes rather than a law, it says that $\rank\Rec$ is a
...
\begin{remark}[a failed certificate is a statement about the elimination]\label{rem:order}
Proposition~\ref{prop:cert} is one-sided: it certifies freeness when the reduction completes
...
Remark~\ref{rem:detector} --- that there is no $2$- and no $3$-torsion. Taking instead the
...
Table~\ref{tab:real} became certificates, and the three below them became feasible at all.
...
lost and the nerve is cut into more components than it should be. The rank of $\Rec$ reported
...
\paragraph{Scope.} Proposition~\ref{prop:sheaf}, Theorem~\ref{thm:mv},
Corollary~\ref{cor:two} and Proposition~\ref{prop:cert} hold for arbitrary strains;
Proposition~\ref{prop:rp2} is a construction; Section~\ref{sec:real} is a computation on
named genomes and claims nothing beyond them.
The pooled graph is single-stranded, as in \cite{Bio9}; the reverse-complement
...
standard, e.g.\ \cite[VII.4]{Brown}; Theorem~\ref{thm:mv} is its two-row case read off for
...
\paragraph{Open.} \emph{First}, torsion in a real pan-genome. Section~\ref{sec:real} answers
this for the \emph{E.\ coli} chromosomes pooled there --- $\Rec$ is free, with a certificate
...
homology rather than strain-specific sequence; by the last paragraph of Section~\ref{sec:real}
this changes the rank of $\Rec$ for divergent strains, and Bio~7's anti-automorphism theorem
...
pooled flow, whose stalk at a bubble is the homogeneous space of \cite{Bio9}; it is not a
sheaf of abelian groups.

\begin{thebibliography}{99}

\bibitem{Bio9} R.~Chen, \emph{Polyploid phasing as a $p$-fold decomposition of a balanced
flow, and the homogeneous space of alleles}, companion note (Bio 9, XXII), September 2026.

\bibitem{Brown} K.~S.~Brown, \emph{Cohomology of Groups}, Graduate Texts in Mathematics
\textbf{87}, Springer, 1982.

\bibitem{EPN20} J.~M.~Eizenga, A.~M.~Novak, J.~A.~Sibbesen, S.~Heumos, A.~Ghaffaari,
G.~Hickey, X.~Chang, J.~D.~Seaman, R.~Rounthwaite, J.~Ebler, M.~Rautiainen, S.~Garg,
B.~Paten, T.~Marschall, J.~Sir\'en and E.~Garrison,
\emph{Pangenome graphs},
Annu.\ Rev.\ Genomics Hum.\ Genet.\ \textbf{21} (2020), 139--162.

\bibitem{GSN18} E.~Garrison, J.~Sir\'en, A.~M.~Novak, G.~Hickey, J.~M.~Eizenga, E.~T.~Dawson,
W.~Jones, S.~Garg, C.~Markello, M.~F.~Lin, B.~Paten and R.~Durbin,
\emph{Variation graph toolkit improves read mapping by representing genetic variation in the
reference}, Nature Biotechnology \textbf{36} (2018), 875--879.

\bibitem{HG19} J.~Hansen and R.~Ghrist,
\emph{Toward a spectral theory of cellular sheaves},
J. Appl.\ Comput.\ Topol.\ \textbf{3} (2019), 315--358.

\bibitem{PNE18} B.~Paten, J.~M.~Eizenga, Y.~M.~Rosen, A.~M.~Novak, E.~Garrison and G.~Hickey,
\emph{Superbubbles, ultrabubbles, and cacti},
J. Comput.\ Biol.\ \textbf{25} (2018), no.~7, 649--663,
\href{https://doi.org/10.1089/cmb.2017.0251}{doi:10.1089/cmb.2017.0251}.
