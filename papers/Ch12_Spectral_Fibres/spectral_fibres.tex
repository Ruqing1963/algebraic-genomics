...
double cover \cite{AG7,AG8,AG9}; that work is collected in the monograph \cite{AG}, which also contains
a version of the present paper as its Chapter~12 (pp.~89--99). For single-stranded linear sequences the
...
\cite[Theorem~7.6 and Remark~7.7, p.~52]{AG7}.
\end{remark}
...
\begin{lemma}[forced arcs {\cite[Lemma~9.7, p.~71]{AG9}}]\label{lem:forced}
...
\cite[Theorem~7.6, p.~52]{AG7}.
\end{proposition}

As in \cite[Definition~7.3, p.~51]{AG7}, $\DS(S,k)$ is defined by the $(k+1)$-mer spectra of both
...
\cite[Corollary~10.6, p.~78]{AG10}, so no closed formula exists unless $\mathrm{FP}=\#\mathrm{P}$.
...
\cite[Section~8.5 and Table~8.2, p.~63]{AG8}.
The number of double-stranded reconstructions is
\[
|\DS(\phi\mathrm{X}174,10)|=31\,925\,753\,246\,212
\]
\cite[Section~9.8 and Table~9.1, pp.~72--73]{AG9}. A strand-agnostic local DNA encoder of window
...
\cite[Theorem~11.1, p.~84]{AG11}. Transpositions between direct repeats and inversions between
...
\cite[Section~11.4 and Conjecture~11.2, pp.~85--86]{AG11}.
...
script, its transcript and the figure script are in the repository of \cite{AG},
\url{https://github.com/Ruqing1963/algebraic-genomics}, folder
\texttt{papers/Ch12\_Spectral\_Fibres}. The double-stranded count of Section~\ref{sec:dna} is reproduced
by \texttt{ds\_fermion.py} and \texttt{ds\_count.py}, the scripts of \cite{AG9}, in the same
repository.
...
\bibitem{AG7} R.~Chen, \emph{Double-stranded genome assembly as a lattice sum, and the inversion-flip group},
Chapter~7 of \cite{AG},
pp.~49--57.
\bibitem{AG8} R.~Chen, \emph{The double-stranded lattice sum by frontier elimination}, Chapter~8 of \cite{AG}, pp.~59--65.
\bibitem{AG9} R.~Chen, \emph{The double-stranded lattice sum as a fermionic partition function},
Chapter~9 of
\cite{AG}, pp.~67--73.
\bibitem{AG10} R.~Chen, \emph{Why there is no closed formula for $\#\mathrm{DS}$: double-stranded reconstruction
is $\#$P-hard}, Chapter~10 of \cite{AG},
pp.~75--79.
\bibitem{AG11} R.~Chen, \emph{Inversions at inverted repeats and the double-stranded fibre}, Chapter~11 of \cite{AG}, pp.~83--88.
\end{thebibliography}
