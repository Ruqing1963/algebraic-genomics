...
algebraic genomics. In earlier work we counted the circular sequences that share a double-stranded
$k$-mer spectrum with a genome, by a lattice sum over the circulations of a de Bruijn double cover
\cite{Bio8,Bio12,Bio13}. For single-stranded linear sequences the same count is a single BEST
...
copies of the same string. The same constant appears in Bio~8's count
$N_{\mathrm{seq}}(c)=t(G_c)\operatorname{Loc}(G_c)/\prod_ac(a)!$ for circular sequences \cite{Bio8}.
\end{remark}
...
\begin{lemma}[forced arcs \cite{Bio13}]\label{lem:forced}
...
Both are \emph{molecular fibres} in the sense of this note: sets of distinct perturbagens that a local
...
on the circulations of the double cover $D_k$, inside the box $|f|\le m$ \cite{Bio8}.
\end{proposition}
...
and the box has $2\,481\,687\,900$ points, of which $909\,103\,210$ have connected support \cite{Bio12}.
The number of double-stranded reconstructions is
\[
|\DS(\phi\mathrm{X}174,10)|=31\,925\,753\,246\,212
\]
\cite{Bio13}. A strand-agnostic local DNA encoder of window $10$ assigns all of them the same
...
\bottomrule
\end{tabular}
\caption{The results of this note set against the corresponding statements of UniPert-G2CP.}\label{tab:vs}
\end{table}
...
double-stranded count of Section~\ref{sec:dna} is reproduced by the ancillary files of \cite{Bio13}.
