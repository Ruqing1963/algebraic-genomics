% Double-stranded genome assembly as a lattice sum, and the inversion-flip group   (Bio 8 / XXI)
% Build: pdflatex anccft21 (x3)
...
\title{\bfseries Double-stranded genome assembly as a lattice sum,\\
and the inversion-flip group}
...
the BEST theorem \cite{AEdB,SmithTutte}. The first companion note \cite{Bio6} passed to the
...
verbatim but that its involution $\rho(v)=\bar v$ reverses arcs. The second \cite{Bio7} proved
...
the sequence count of the multigraph at each point (Theorem~\ref{thm:lattice}). The rank of
the lattice has a closed form (Theorem~\ref{thm:rank}), and the sum, evaluated for $\phi$X174,
gives $10$ and $864$ where $|\K(D_k)|$ gives $33$ and $25\,318\,912$ (Table~\ref{tab:lattice}).
...
flip graph on all $864$ reconstructions at $k=11$ is connected (Theorem~\ref{thm:flips}),
...
Remark~\ref{rem:corrections}: the rank of $\Lm$ is not the number of inverted repeats; the
...
$D_k=D_k(S)$ is the multidigraph of \cite{Bio6}: one vertex per $k$-mer occurring at least
...
(b) The vertex statement is \cite[Prop.~2]{Bio7}. A content with two parallel copies is a
...
(c) is \cite[Thm.~3]{Bio6}: all $k$-mers of $S$ are joined by the closed walk $S$, all those of
...
of Table~\ref{tab:rank} to the other as $k$ changes parity.
...
\begin{theorem}[The lattice sum]\label{thm:lattice}
...
\begin{proof}
Lemma~\ref{lem:spectrum} puts $\spec(T)$ in $P(S,k)$; the support is connected because $T$ is
...
For the fibre, the BEST theorem \cite{AEdB,SmithTutte} counts the Eulerian circuits of
...
\begin{remark}[The normalisation is not optional]\label{rem:norm}
...
quoted for $\phi$X174 at $k=11$ in \cite{Bio1}, $|\K(G_{11})|\Loc=132$, is the arc-rooted
count; the number of circular sequences with the spectrum of $S$ is $132/4=33$, and $33$ is
what appears at the point $c=\spec(S)$ of Table~\ref{tab:lattice}.
\end{remark}

\begin{remark}[Where the linking form went]
The symmetric linking form of \cite[Rem.~5]{Bio7} does not enter Theorem~\ref{thm:lattice},
...
\begin{theorem}[Rank of the anti-invariant lattice]\label{thm:rank}
...
permutation matrices of $\rho$ on contents and vertices. By Lemma~\ref{lem:rho}(a),
...
(Lemma~\ref{lem:rho}(c)). Hence $\tr(\rho|Z_1)=\#\mathrm{fix}_A+\#\mathrm{fix}_V-
...
\begin{proof}
The first two statements are Theorem~\ref{thm:rank} with Lemma~\ref{lem:rho}(b). If $D_k$
...
Theorem~\ref{thm:rank} in every row. At $k=13$ no $k$-mer is repeated on either strand and
$D_k$ is the pair of disjoint cycles $S,\bar S$. Check (L).}
\label{tab:rank}
\end{table}

\begin{remark}[Not the number of inverted repeats]\label{rem:notIR}
It is tempting to read $\rank\Lm$ as the number of independent inversions available to the
genome, and both the specification of this note and its planning did. Table~\ref{tab:rank}
...
repeated $12$-mer, and $\rank\Lm=4$. At the other extreme, Corollary~\ref{cor:parity} says
...
connected box points, $8$ and $218$ in Table~\ref{tab:lattice}, and it has no closed form we
know of.
\end{remark}
...
Theorem~\ref{thm:lattice}; ``brute'' the number of closed walks of $D_k$ with
...
$\bar w$, is a flip in the sense of Definition~\ref{def:flip}.
...
flips everything. (d) By Lemma~\ref{lem:rho}(c) $D_k$ is then disconnected, $T$ lies in one
...
\begin{theorem}[$\phi$X174, computed]\label{thm:flips}
At $k=12$ and $k=11$:
\begin{enumerate}
...
brute force of check (E), all flips are generated from Definition~\ref{def:flip}, and each
...
\begin{remark}[Corrections to the specification]\label{rem:corrections}
...
repeats of length $\ge k$ (Remark~\ref{rem:notIR}); it is given by Theorem~\ref{thm:rank}.
\emph{Second}, the per-point term is not $t(c_f)\prod(d^+-1)!$ but that quantity divided by
$\prod_a c_f(a)!$ (Remark~\ref{rem:norm}); the un-normalised sum is $2756$ where the count is
...
computation, and not theorems: Proposition~\ref{prop:flip}(d) gives the general obstruction,
...
\paragraph{Scope.} Lemma~\ref{lem:rho}, Lemma~\ref{lem:spectrum}, Theorem~\ref{thm:lattice},
Theorem~\ref{thm:rank}, Corollary~\ref{cor:parity} and Proposition~\ref{prop:flip} are proved
in full generality for a single circular sequence. Theorem~\ref{thm:flips} is a computation
...
Table~\ref{tab:rank} is reported there. The count is a count of circular sequences, each
weighted by $1/d(T)$; in the data $d(T)=1$ throughout.

\paragraph{Provenance.} The BEST theorem is \cite{AEdB,SmithTutte}; its use to count genome
...
\cite{MGMB07} after \cite{KM95}, and those papers ask for existence and for one solution, not
...
\cite{Bio6}; the anti-automorphism theorem and the symmetric linking form are \cite{Bio7}; the
single-strand counting convention is \cite{Bio1}. The Hopf trace formula is standard. We are
...
Theorem~\ref{thm:rank} evaluates the rank of $\Lm$ for \emph{E.\ coli} in one line from the
double cover of \cite{Bio6}, but the sum is not enumerable there, and what is wanted is a
...
Theorem~\ref{thm:flips}(c). \emph{Fourth}, the same lattice with the involution removed and
...
\bibitem{BP96} V.~Bafna and P.~A.~Pevzner,
\emph{Genome rearrangements and sorting by reversals},
SIAM J. Comput. \textbf{25} (1996), no.~2, 272--289.
...
double cover of a de Bruijn graph}, companion note (Bio 6), September 2026.

\bibitem{Bio7} R.~Chen, \emph{The reverse-complement quotient is a signed graph: what
Zaslavsky's theory gives, and what it cannot}, companion note (Bio 7), September 2026.

\bibitem{HP99} S.~Hannenhalli and P.~A.~Pevzner,
\emph{Transforming cabbage into turnip: polynomial algorithm for sorting signed permutations
by reversals}, J. ACM \textbf{46} (1999), no.~1, 1--27.

\bibitem{KM95} J.~D.~Kececioglu and E.~W.~Myers,
