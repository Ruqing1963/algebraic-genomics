...
is the $r$-rank, and exactly so: modulo a prime $p$ dividing $r$ an interior vertex of the
chain has degree and adjacencies all $\equiv0$, so its row and column of the signed Laplacian
vanish and $\Sig$ and $\Sig/C$ reduce to the same matrix. That one observation --- a
\emph{$p$-divisible subgraph is invisible mod $p$} --- proves the $p$-rank gain to be exactly
$s$ for every prime $p\mid r$, composite $r$ included, and it is also what the
$\mathbb{F}_7$-nullity below measures.
The seven ribosomal operons of \emph{E.\ coli} K-12 merge into genuine
...
Theorem~\ref{thm:series}(a) is visible, and Theorem~\ref{thm:prank} says it must be exactly
$s$ --- but the order ratio never equals $r^s$, and is not even an integer in five rows.
Check (C); check (Q) extends the $p$-rank column to composite $r=4,6,9,10,12,15,21,25$ and
to $p=2$, and to $p$-divisible trees, cycles and multi-attachment subgraphs.}
\label{tab:series}
\end{table}

Table~\ref{tab:series} separates the two halves of the theorem. What does not survive
contraction is the order and the isomorphism type. What survives is the $r$-\emph{rank}, and
the reason is a one-line observation that has nothing to do with chains.

\begin{lemma}[$p$-divisible subgraphs are invisible mod $p$]\label{lem:pdiv}
Let $\Sig$ be a signed multigraph, $p$ a prime, and $H$ a set of bundles of $\Sig$ every one
of which has multiplicity divisible by $p$. Then
\[
  \rank_{\mathbb F_p}\Delta_\Sig=\rank_{\mathbb F_p}\Delta_{\Sig\setminus H},
\]
where $\Sig\setminus H$ is $\Sig$ with the bundles of $H$ deleted and the vertex set kept.
\end{lemma}

\begin{proof}
Call a vertex \emph{interior} to $H$ if every bundle at it lies in $H$. Its degree is a sum
of multiplicities divisible by $p$, hence $\equiv0$, and each of its adjacency entries is
$\pm$ a multiplicity in $H$, hence $\equiv0$: its whole row and column of $\Delta_\Sig$
vanish mod $p$, and they vanish in $\Delta_{\Sig\setminus H}$ too, that vertex now being
isolated. A vertex not interior to $H$ loses, on passing to $\Sig\setminus H$, exactly the
$H$-part of its degree and its $H$-adjacencies; both are $\equiv0$. So the two matrices are
congruent mod $p$ entry by entry.
\end{proof}

\begin{theorem}[the $r$-rank gain, exactly]\label{thm:prank}
Let $\Sig$ be connected and unbalanced with an $r$-bundle chain $C$ of $s$ internal vertices,
and let $\Sig/C$ be connected and unbalanced as well. Then for every prime $p$ dividing $r$,
\[
  p\text{-}\rank\bigl(\coker\Delta_\Sig\bigr)-p\text{-}\rank\bigl(\coker\Delta_{\Sig/C}\bigr)=s ,
\]
and for every \emph{odd} prime $p\mid r$ the same holds with $\coker\Delta$ replaced by the
Reiner--Tseng group $\K$. More generally, if $H$ is any $p$-divisible subgraph with $w$
interior vertices then the $p$-rank of $\coker\Delta$ exceeds that of
$\coker\Delta_{\Sig\setminus H}$ by exactly $w$.
\end{theorem}

\begin{proof}
Both the chain's bundles in $\Sig$ and the single $a$--$b$ bundle of $\Sig/C$ have
multiplicity $r$, so Lemma~\ref{lem:pdiv} applied to each deletes them and leaves the same
matrix $\Delta_{\Sig'}$, $\Sig'$ being $\Sig$ with the chain and both end bundles removed.
Hence $\rank_{\mathbb F_p}\Delta_\Sig=\rank_{\mathbb F_p}\Delta_{\Sig/C}$, while $\Delta_\Sig$
is larger by the $s$ interior vertices, so its nullity mod $p$ exceeds that of
$\Delta_{\Sig/C}$ by $s$. For a connected unbalanced signed graph $\Delta$ is positive
definite, hence nonsingular, so $\coker\Delta$ is finite and its $p$-rank --- the number of
invariant factors divisible by $p$ --- equals $\dim_{\mathbb F_p}\coker\Delta\otimes\mathbb F_p
=\operatorname{nullity}_{\mathbb F_p}\Delta$. For odd $p$, $\K(\Sig)$ has index two in
$\coker\Delta_\Sig$ and therefore the same $p$-primary part. The general statement is the
same argument with $s$ replaced by the number of interior vertices of $H$.
\end{proof}

\begin{remark}[what happens at $p=2$]\label{rem:ptwo}
For $p=2$ the index-two subgroup does matter. Tensoring
$0\to\K(\Sig)\to\coker\Delta_\Sig\to\Z/2\to0$ with $\mathbb F_2$ gives
$2\text{-}\rank\K=2\text{-}\rank\coker\Delta-1+\varepsilon$ with $\varepsilon\in\{0,1\}$
recording whether the extension splits at $2$, so the gain for $\K$ is
$s+\varepsilon_\Sig-\varepsilon_{\Sig/C}$. In the basis
$2e_0,\ e_0+e_1,\dots,e_0+e_{n-1}$ of $\im\partial$ the chain's column reduces mod $2$ to a
single possibly nonzero entry in the row of $2e_0$, which bounds the deviation by one. In
$5828$ connected unbalanced instances with even $r$ the two $\varepsilon$ agree and the gain
is $s$; that they always agree is \emph{not} proved.
\end{remark}

Theorem~\ref{thm:prank} is a partial and negative data point for Question 5.5 of
\cite{CDIOSZ}, which asks how $\K(\Sig_\varphi)$ depends on $\K(|\Sig|)$ as the signature
varies and observes only that the number of invariant factors seems to be preserved: here the
count of factors divisible by $p$ is pinned exactly, while the order and the isomorphism type
are not.
...
Lemma~\ref{lem:pdiv} at $p=7$: the union of all $7$-bundles is a $7$-divisible subgraph,
hence invisible over $\mathbb{F}_7$, and every vertex interior to it --- every vertex whose
bundles are all $7$-bundles --- contributes a zero row and a unit of nullity. So the
$\mathbb{F}_7$-nullity and the exactness of the chain's $7$-rank gain are two readings of one
lemma, and the numbers below are an instance of Theorem~\ref{thm:prank} at genome scale.
The $\mathbb{F}_7$-nullity therefore measures the
...
extension arising from a real genome. Lemma~\ref{lem:pdiv} and Theorem~\ref{thm:prank} are
proved in full; an earlier version of this note had the exactness of the $r$-rank gain as a
computational observation only. Lemma~\ref{lem:pdiv} is elementary enough that we would not
be surprised to find it in the literature on $p$-ranks of critical groups, and we claim
novelty only for its two consequences here. What remains unproved is the case $p=2$ for the
Reiner--Tseng group: by Remark~\ref{rem:ptwo} the gain is $s+\varepsilon_\Sig-
\varepsilon_{\Sig/C}$, and that the two $\varepsilon$ always agree is verified on $5828$
instances and not proved.
...
\emph{First}, the residue of Theorem~\ref{thm:prank} at the prime two: show that the
index-two extension of Remark~\ref{rem:ptwo} splits, or fails to split, for $\Sig$ and
$\Sig/C$ together, so that the gain for the Reiner--Tseng group is $s$ there as well --- or
exhibit the pair where it is $s\pm1$.
