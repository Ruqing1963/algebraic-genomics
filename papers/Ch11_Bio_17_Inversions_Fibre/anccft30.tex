% Inversions at inverted repeats and the double-stranded fibre   (Bio 17 / XXX)
% Build: pdflatex anccft30 (x2)
...
\title{\bfseries Inversions at inverted repeats\\ and the double-stranded fibre}
...
is the double-stranded fibre $\DS(S,k)$ counted in Bio~8, 12 and~13. Bio~8 identified the move that
...
genome has no inverted repeat. We add the lattice picture: a flip moves the point of Bio~8 by
...
enumeration and against Bio~8's lattice sum, and $\phi$X174 at $k=12$ and $k=11$, where the closure of
the genome is exactly $\#\DS=10$ and $864$. Transpositions alone reach $2$ and $33$ sequences there,
and adding reverse complementation reaches $4$ and $66$. Adding transpositions removes Bio~8's
...
about $5\,000$ of its lattice points. The inversion-group programme proposed for this paper (Artin
groups, Garside normal forms) is set aside, and we say why.
\end{abstract}

\medskip
\noindent\textbf{Keywords.} de Bruijn graph, reverse complement, inverted repeats, chromosomal
inversions, Eulerian circuits, transpositions, Tate cohomology, genome assembly.
...
\]
and the set $\DS(S,k)$ of circular $T$ satisfying this is larger. Bio~8 wrote its size as a sum over the
points of a lattice $\Lm$ in a box, Bio~12 counted the box, and Bio~13 computed
$\#\DS(\phi\mathrm X174,10)=31\,925\,753\,246\,212$ \cite{Bio8,Bio12,Bio13}. A transposition never
leaves the lattice point of $T$. So the question of this note is \emph{which moves carry one lattice point
to another}, and whether together with transpositions they connect the fibre.

The move itself is Bio~8's \emph{inversion flip} \cite[\S7]{Bio8}: an inversion between two copies of an
...
repeats \cite{SL02}, and here it is the combinatorial content of the second strand. Bio~8 proved two
...
flips alone connect all $10$ and $864$ reconstructions. This note adds three things. It combines flips
...
We keep Bio~8's notation. $D_k$ is the labelled reverse-complement double cover of $S$. Its vertices are
the $k$-mers that occur at least twice in $S$ and $\rc S$ together. Its arcs, the \emph{contents}, are
the stretches between consecutive such $k$-mers. The involution $\rho$ is reverse complementation, on
vertices ($v\mapsto\bar v$) and on contents; a content $a\colon t\to h$ goes to
$\rho a\colon\bar h\to\bar t$. A reconstruction $T\in\DS(S,k)$ is an Eulerian circuit of the multigraph
with $c(a)$ copies of each content, where $c+\rho c=m$ and $m$ is the multiplicity vector of $D_k$. With
$f=2c-m$ one has $f\in\Lm=\{f\in Z_1(D_k):\rho f=-f\}$ inside the box $|f|\le m$, and
\[
\#\DS(S,k)=\sum_f\Nseq\bigl(\tfrac{m+f}2\bigr)
\]
\cite{Bio8}. Here $(\rho z)(a)=z(\rho a)$ on $1$-chains. A vertex is \emph{palindromic} if $\bar v=v$, and
a content is \emph{self-complementary} if $\rho a=a$.

\section{Inversions at inverted repeats}\label{sec:inv}

\begin{theorem}[flips preserve the double-stranded spectrum {\cite[Prop.~(flip)(a)]{Bio8}}]\label{thm:inv}
...
This is Bio~8's Proposition on the inversion flip, part~(a), and its proof is Bio~8's. We repeat it only
to fix notation; in Bio~8's words, the move of this section is a \emph{flip}.
...
Check (I) of \texttt{bio17\_verify.py} tests Theorem~\ref{thm:inv} directly on strings. All $156$
...
\section{Which moves generate the fibre}\label{sec:gen}
...
segments;
\item \textbf{I}: the inversions of Theorem~\ref{thm:inv}, at every pair of visits to $v$ and $\bar v$;
...
exhaustive search. The second is Bio~8's lattice sum. The third is the closure of $S$ under T, I and R.
...
Bio~8 and Bio~12.}\label{tab:gen}
\end{table}

For $\phi$X174 these closures refine Bio~8's Theorem on flips, which showed that at $k=12$ and
$k=11$ the flips \emph{alone}, without transpositions and without reverse complementation, already
connect the fibre \cite[\S7]{Bio8}. What transpositions add is the general case: Bio~8's
Proposition~(flip)(d) shows that a genome without inverted repeats admits no flip at all.
Table~\ref{tab:gen} separates the contributions. Transpositions alone give the single-stranded fibre of
...
one random case of twenty-four, T and I reach only part of the fibre and R is needed as well. By Bio~8's
Proposition~(flip)(c), reverse complementation is itself a flip only at a palindromic visit. We have not
checked whether the exceptional case lacks one.

\begin{conjecture}[double-stranded Ukkonen--Pevzner]\label{conj:gen}
...
Within one lattice point the conjecture is the classical theorem. The open part is the passage
...
current circuit. Section~\ref{sec:lattice} shows that nothing is lost at the level of the lattice.

\section{The inversion lattice}\label{sec:lattice}

By \eqref{eq:move} the inversion vectors $(1-\rho)z$, for all paths $z$ from a vertex to its complement,
...
\begin{theorem}[the Tate dichotomy]\label{thm:tate}
...
always equal $\operatorname{rank}\Lm$: $48$, $12$ and $4$ for $\phi$X174.}\label{tab:tate}
\end{table}

Table~\ref{tab:tate} confirms the theorem exactly on every cover tested. The dichotomy has a transparent
...
\cite{Bio13}. It cannot be closed, but it can be walked. The genome carries $31$ inversions at inverted
...
Conjecture~\ref{conj:gen} but does not test it: at this scale the question is not whether the walk can
move, but whether it can reach every one of the $454\,551\,605\times2$ points with connected support.

\section{Biological reading}\label{sec:bio}

Theorem~\ref{thm:inv} says exactly which rearrangements double-stranded $k$-mer data cannot see:
...
a reverse-complement-invariant local DNA encoder of window $k+1$, in the sense of our companion note on
spectral fibres \cite{CC26}. A short-read assembler faces the same limit. The inversion lattice being
...
\section{On the proposed inversion-group programme}\label{sec:programme}

This paper was specified with a different programme: a group $G_{\mathrm{inv}}$ generated by adjacent
...
repeats, and they are what this note studies.
\end{itemize}

\section*{Ancillary file}
\texttt{bio17\_verify.py} (standard library; imports \texttt{ds\_fermion.py} of Bio~13) runs checks
...
double cover} (Bio~8 / XXI).
\bibitem{Bio12} R.~Chen, \emph{The double-stranded lattice sum by frontier elimination} (Bio~12 / XXV).
...
\bibitem{HP99} S.~Hannenhalli, P.~A.~Pevzner, \emph{Transforming cabbage into turnip: polynomial
algorithm for sorting signed permutations by reversals}, J.\ ACM \textbf{46} (1999) 1--27.
\bibitem{Bro82} K.~S.~Brown, \emph{Cohomology of Groups}, Graduate Texts in Mathematics~87, Springer
(1982).
\bibitem{SL02} P.~Stankiewicz, J.~R.~Lupski, \emph{Genome architecture, rearrangements and genomic
disorders}, Trends Genet.\ \textbf{18} (2002) 74--82.
\end{thebibliography}
