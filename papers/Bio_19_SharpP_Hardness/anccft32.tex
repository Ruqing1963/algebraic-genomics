% #DS is #P-hard   (Bio 19 / XXXII)
% Build: pdflatex anccft32 (x2)
...
\title{\bfseries Why there is no closed formula for $\#\DS$:\\
double-stranded reconstruction is $\sP$-hard}
...
double-stranded count $\#\DS(S,k)$ of Bio~8 is a sum of such determinants over the points of a lattice
in a box. Bio~12 and~13 evaluated it for $\phi$X174 by frontier elimination and by visiting
...
corollaries follow. The number of box points, Bio~12's first column, is the number of Eulerian
orientations of $G$, which is $\sP$-complete as well. And on this class the generation conjecture of
Bio~17 holds, by Kotzig's theorem. All identities are verified exactly on $23$ graphs by
...
\medskip
\noindent\textbf{Keywords.} computational complexity, $\sP$-completeness, Eulerian circuits, Eulerian
orientations, BEST theorem, de Bruijn graph, reverse complement, palindromes, genome assembly.
...
factorials \cite{BEST}. For two strands, Bio~8 wrote
\[
\#\DS(S,k)=\sum_{f\in\Lm,\ |f|\le m,\ f\equiv m\ (2)}\Nseq\bigl(\tfrac{m+f}2\bigr)
\]
\cite{Bio8}. Bio~12 counted the $2\,481\,687\,900$ points of the box for $\phi$X174 at $k=10$, and Bio~13
evaluated the sum, $31\,925\,753\,246\,212$ \cite{Bio12,Bio13}. Each step needed an exponential-size
object: a frontier of width $26$, or a visit to every point. A proposal for this paper asked for a
``meshless'' formula instead. It would be a Gauss sum built on the symmetric linking form of the double
cover's sandpile group \cite[Rem.~5]{Bio7}, or a Pfaffian in a Clifford algebra, evaluated in time polynomial in the
number of vertices. We prove that such a formula cannot exist unless every problem in $\sP$ is solvable
in polynomial time.

\section{Eulerian circuits and their orientations}\label{sec:euler}
...
\begin{theorem}[\cite{BW05,GS12}]\label{thm:sharpP}
...
\section{Genomes whose double cover is a given graph}\label{sec:real}
...
\begin{proof}
The vertices of $D_k$ are the repeated $k$-mers \cite{Bio8}; $P_x$ occurs $\deg(x)\ge2$ times. The contents
...
\begin{lemma}[the hypotheses can be met in polynomial time]\label{lem:derand}
There are $h,\ell=O(\log m)$ and a deterministic algorithm, polynomial in $m$, that chooses the words $u_x$
and the spacer interiors so that the hypotheses of Lemma~\ref{lem:real} hold.
\end{lemma}
...
For $S$ and $k$ as in Section~\ref{sec:real},
\[
\#\DS(S,k)=2\,\varepsilon(G),
\]
and the box points of Bio~8 are in bijection with the Eulerian orientations of $G$.
\end{theorem}
...
$\Nseq=\tau\prod(d^+-1)!/\prod c(a)!$ disappear and $\Nseq=\ec(O)$ \cite{Bio8}. Summing gives
Lemma~\ref{lem:orient}.
\end{proof}

\begin{corollary}\label{cor:hard}
Given a circular DNA sequence $S$ and an even $k$, computing $\#\DS(S,k)$ is $\sP$-hard, and so is
computing the number of points of Bio~8's box. In particular, no algorithm, closed formula or
...
Theorem~\ref{thm:sharpP}. For the box, use \cite{MW96}.
\end{proof}
...
enumeration of trails. $\#\DS$ comes from Bio~8's lattice sum on the double cover built from the
definition, and it equals $2\varepsilon(G)$ in every case. So does the closure of $S$ under Bio~17's
moves. The box points equal the Eulerian orientations in every case.}\label{tab:res}
\end{table}

Table~\ref{tab:res} shows a selection of the verified cases. In every case, the double cover of the
...
\section{Two by-products}\label{sec:by}

\paragraph{Bio~17's conjecture on this class.} Bio~17 conjectured that transpositions, inversions at
inverted repeats, and reverse complementation connect $\DS(S,k)$ \cite{Bio17}. Here every vertex is its
own complement. So an inversion between two visits of $P_x$ (a flip in the sense of
\cite[\S7]{Bio8}) reverses the closed stretch of the circuit between them, which is a
...
\begin{corollary}
The generation conjecture of Bio~17 holds for every genome of Section~\ref{sec:real}.
\end{corollary}

Check~(K) confirms it: the closure of $S$ under inversions and reverse complementation alone is all of
$\DS(S,k)$ in all $23$ cases.

\paragraph{Both counting columns are hard.} Bio~12 counted the points of the box ($2\,481\,687\,900$ for
$\phi$X174 at $k=10$) and Bio~13 the reconstructions. By Theorem~\ref{thm:main} the first count is the
...
\section{What this does and does not exclude}\label{sec:scope}
...
\item \emph{Not excluded:} algorithms exponential in a structural parameter. Bio~12's frontier
...
$k$ no vertex is its own complement, and the quotient of $D_k$ is the signed graph of Bio~7 \cite{Bio7}.
...
\texttt{bio19\_verify.py} (standard library; imports \texttt{ds\_fermion.py} of Bio~13 and
\texttt{bio17\_verify.py} of Bio~17) runs checks (E), (R), (D), (K) and (S) in two seconds. It writes
its transcript to \texttt{bio19\_output.txt}.

\begin{thebibliography}{99}
\bibitem{Bio7} R.~Chen, \emph{The reverse-complement quotient is a signed graph: what Zaslavsky's theory
gives, and what it cannot} (Bio~7).
...
(Bio~13 / XXVI).
\bibitem{Bio17} R.~Chen, \emph{Inversions at inverted repeats and the double-stranded fibre}
(Bio~17 / XXX).
...
\bibitem{BW05} G.~R.~Brightwell, P.~Winkler, \emph{Counting Eulerian circuits is \#P-complete}, in
Proc.\ 7th ALENEX and 2nd ANALCO (2005) 259--262.
\bibitem{GS12} Q.~Ge, D.~\v Stefankovi\v c, \emph{The complexity of counting Eulerian tours in
$4$-regular graphs}, Algorithmica \textbf{63} (2012) 588--601.
\bibitem{MW96} M.~Mihail, P.~Winkler, \emph{On the number of Eulerian orientations of a graph},
Algorithmica \textbf{16} (1996) 402--414.
\bibitem{Kot68} A.~Kotzig, \emph{Eulerian lines in finite $4$-valent graphs and their transformations},
in Theory of Graphs (Proc.\ Colloq.\ Tihany, 1966), Academic Press (1968) 219--230.
\bibitem{Fle90} H.~Fleischner, \emph{Eulerian Graphs and Related Topics}, Part~1, Vol.~1, Annals of
Discrete Mathematics~45, North-Holland (1990).
\end{thebibliography}
