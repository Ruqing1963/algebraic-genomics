% The sandpile group as a coordinate system on genome reconstructions
% Build: pdflatex rotor_assembly (x3)
...
\title{\bfseries The sandpile group as a coordinate system\\ on genome reconstructions}
...
Eulerian circuits; Creed and Cryan state this explicitly \cite{CC}. (For \emph{undirected}
...
(Theorem~\ref{thm:torsor}): the set of reconstructions, modulo the local exit choices, is
a torsor for $\K(G)$. Consequences: a chip placed on a $k$-mer is an explicit
rearrangement between two candidate genomes (Section~\ref{sec:moves}); the difference of
...
(Section~\ref{sec:sampling}). Section~\ref{sec:phix} applies this to $\varphi$X174, where
...
\cite{CMN,PW,CC} and, in bioinformatics, \cite{AE,KMUW,uShuffle}. The contribution is the
...
\section{The torsor}\label{sec:torsor}

\begin{theorem}[Coordinates on reconstructions]\label{thm:torsor}
...
statement about genomes, and the two verifications of Section~\ref{sec:battery}: check~(A)
...
\begin{remark}[Why the quotient]\label{rem:quotient}
...
invisible to branch statistics; Theorem~\ref{thm:torsor} says it nevertheless has the
structure of a finite abelian group, given explicitly by the Smith normal form of the
out-Laplacian.
\end{remark}

\section{A chip is a rearrangement}\label{sec:moves}
...
\begin{proposition}[Distance]\label{prop:distance}
...
Proposition~\ref{prop:distance} is therefore not the transposition or reversal distance
of comparative genomics, and we see no immediate inequality between them.

\section{Sampling, and what the group adds}\label{sec:sampling}
...
orders uniformly \cite{CC}; this is what \cite{AE,KMUW} do and what uShuffle implements
\cite{uShuffle}. Theorem~\ref{thm:torsor} gives the same distribution in a different form:
...
\begin{remark}[Local constraints are not cosets]\label{rem:cosets}
...
\section{$\varphi$X174: one global choice}\label{sec:phix}
...
\cite[Tab.~1]{Bio1}. Theorem~\ref{thm:torsor} says that the $12$-mer spectrum of the phage
leaves exactly \emph{two} global reconstructions. Check~(F) exhibits them.

\begin{proposition}[$\varphi$X174 at $k=12$]\label{prop:phix}
...
The script \texttt{rotor\_assembly.py} runs the eight checks of Table~\ref{tab:battery} in
...
key & check & mode\\
\midrule
(A) & Theorem~\ref{thm:torsor}(i) on six assembly graphs, $|\K|\le27$: free and transitive & exact\\
(B) & Theorem~\ref{thm:torsor}(ii): the walk is a bijection onto the enumerated circuits & exact\\
(M) & Section~\ref{sec:moves}: chips are spectrum-preserving rearrangements & exact\\
...
(F) & Proposition~\ref{prop:phix} & exact\\
(J) & junctions re-threaded per chip on $\varphi$X174 at $k=11$: $0$ to $6$ & exact\\
(N) & Remark~\ref{rem:cosets}: constraint sets are not cosets on $3$ of $4$ graphs & exact\\
\bottomrule
\end{tabular}
...
\cite{CMN,PW,Wilson,CC} and its bioinformatic use \cite{AE,KMUW,uShuffle}; counting
\cite{ABCS,KSP}; the spectrum of $\varphi$X174 and the interleaving theorem \cite{Bio1}.

\emph{New here, as far as we can determine.} Theorem~\ref{thm:torsor}(iii) as a statement
about assemblies; chip-firing as rearrangement and the word metric of
Proposition~\ref{prop:distance}; the coordinate form of the sampler; and
Proposition~\ref{prop:phix}.
...
Proposition~\ref{prop:distance} relates to any edit distance between assemblies; since a
single chip can re-thread many junctions (Section~\ref{sec:moves}), it is not the
transposition distance, and any relation would have to pass through the structure of
rotor walks on the compacted graph.

\section*{Provenance}

Unrefereed preprint. Theorem~\ref{thm:torsor} is a composition of published results and is
verified, not reproved; Proposition~\ref{prop:distance} is immediate from it;
Proposition~\ref{prop:phix} is an exact computation reproducible in seconds. We searched
...
\bibitem{AE} S.~F.~Altschul and B.~W.~Erickson,
\emph{Significance of nucleotide sequence alignments: a method for random sequence
permutation that preserves dinucleotide and codon usage},
Mol.\ Biol.\ Evol.\ \textbf{2} (1985), 526--538.

\bibitem{ABCS} R.~Arratia, B.~Bollob\'as, D.~Coppersmith and G.~B.~Sorkin,
\emph{Euler circuits and DNA sequencing by hybridization},
Discrete Appl.\ Math.\ \textbf{104} (2000), 63--96.
...
\bibitem{BK} H.~Bidkhori and S.~Kishore,
...
\bibitem{BW} G.~R.~Brightwell and P.~Winkler,
\emph{Counting Eulerian circuits is $\#$P-complete},
in: Proc.\ ALENEX/ANALCO 2005, SIAM, 2005, pp.~259--262.
...
\bibitem{CMN} C.~J.~Colbourn, W.~J.~Myrvold and E.~Neufeld,
\emph{Two algorithms for unranking arborescences},
J.\ Algorithms \textbf{20} (1996), 268--281.

\bibitem{CC} P.~Creed and M.~Cryan,
\emph{The number of Euler tours of random directed graphs},
Electron.\ J.\ Combin.\ \textbf{20} (2013), P13.
...
\bibitem{uShuffle} M.~Jiang, J.~Anderson, J.~Gillespie and M.~Mayne,
\emph{uShuffle: a useful tool for shuffling biological sequences while preserving the
$k$-let counts}, BMC Bioinformatics \textbf{9} (2008), 192.

\bibitem{KMUW} D.~Kandel, Y.~Matias, R.~Unger and P.~Winkler,
\emph{Shuffling biological sequences},
Discrete Appl.\ Math.\ \textbf{71} (1996), 171--185.
...
\bibitem{MB} P.~Medvedev and M.~Brudno,
\emph{Maximum likelihood genome assembly},
J.\ Comput.\ Biol.\ \textbf{16} (2009), 1101--1116.

\bibitem{PDDK} V.~B.~Priezzhev, D.~Dhar, A.~Dhar and S.~Krishnamurthy,
\emph{Eulerian walkers as a model of self-organized criticality},
Phys.\ Rev.\ Lett.\ \textbf{77} (1996), 5079--5082.

\bibitem{PW} J.~G.~Propp and D.~B.~Wilson,
\emph{How to get a perfectly random sample from a generic Markov chain and generate a
random spanning tree of a directed graph}, J.\ Algorithms \textbf{27} (1998), 170--217.
...
\bibitem{Wilson} D.~B.~Wilson,
\emph{Generating random spanning trees more quickly than the cover time},
in: Proc.\ 28th ACM Symp.\ Theory of Computing (1996), pp.~296--303.
