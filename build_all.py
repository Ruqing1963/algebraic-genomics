...
H = 3600
CHAPTERS = [
    ("1", "papers/Ch01_Bio_01_Critical_Groups",  "dbg_sandpile.tex",       [dict(file="dbg_sandpile.py", ref="dbg_sandpile_output.txt", quick=[], full=[], timeout=H)]),
    ("2", "papers/Ch02_Bio_02_Rotor_Routing",  "rotor_assembly.tex",     [dict(file="rotor_assembly.py", ref="rotor_assembly_output.txt", quick=[], full=[], timeout=H)]),
    ("3", "papers/Ch03_Bio_03_Repeat_Splitting",  "repeat_splitting.tex",   [dict(file="repeat_splitting.py", ref="repeat_splitting_output.txt", quick=[], full=[], timeout=H)]),
    ("4", "papers/Ch04_Bio_04_Multicopy_Repeats",  "multicopy.tex",          [dict(file="multicopy.py", ref="multicopy_output.txt", quick=[], full=[], timeout=H)]),
    ("15", "papers/Ch15_Bio_05_Ecoli_Decomposition",  "ecoli_decomposition.tex", [dict(file="ecoli_sandpile.py", ref="ecoli_sandpile_output.txt", quick=[], full=[], timeout=H)]),
    ("5", "papers/Ch05_Bio_06_RC_Double_Cover",  "rc_double_cover.tex",    [dict(file="rc_double_cover.py", ref="rc_double_cover_output.txt", quick=[], full=[], timeout=H)]),
    ("6", "papers/Ch06_Bio_07_Signed_Quotient",  "rc_quotient_signed.tex", [dict(file="bidirected_sandpile.py", ref="bidirected_sandpile_output.txt", quick=None, full=[], timeout=2 * H)]),
    ("7", "papers/Ch07_Bio_08_DS_Lattice_Sum",  "anccft21.tex",           [dict(file="ds_lattice.py", ref="ds_lattice_output.txt", quick=[], full=[], timeout=H),
                                                dict(file="ds_flips.py", ref="ds_flips_output.txt", quick=[], full=[], timeout=H)]),
    ("13", "papers/Ch13_Bio_09_Polyploid_Phasing",  "anccft22.tex",           [dict(file="polyploid_phasing.py", ref="polyploid_phasing_output.txt", quick=[], full=[], timeout=H)]),
    ("14", "papers/Ch14_Bio_10_Pangenome_Sheaf", "anccft23.tex",           [dict(file="pangenome_sheaf.py", ref="pangenome_sheaf_output.txt", quick=[], full=[], timeout=H),
                                                dict(file="real_pangenome.py", ref="real_pangenome_output.txt", quick=None, full=[], timeout=H, out_flag="--out")]),
    ("17", "papers/Ch17_Bio_11_Isoform_Polytope", "anccft24.tex",           [dict(file="isoform_polytope.py", ref="isoform_polytope_output.txt", quick=[], full=[], timeout=H)]),
    ("8", "papers/Ch08_Bio_12_Frontier_Elimination", "anccft25.tex",           [dict(file="ds_transfer_matrix.py", ref="ds_transfer_matrix_output.txt", quick=["--quick"], full=[], timeout=4 * H)]),
    ("9", "papers/Ch09_Bio_13_Fermionic_Partition", "anccft26.tex",           [dict(file="ds_fermion.py", ref="ds_fermion_output.txt", quick=["--quick"], full=[], timeout=H),
    ...
    ("16", "papers/Ch16_Bio_14_Snarl_Schur", "anccft27.tex",           [dict(file="bio14_verify.py", ref="bio14_output.txt", quick=[], full=[], timeout=H)]),
    ("11", "papers/Ch11_Bio_17_Inversions_Fibre", "anccft30.tex",           [dict(file="bio17_verify.py", ref="bio17_output.txt", quick=["--quick"], full=[], timeout=H)]),
    ("10", "papers/Ch10_Bio_19_SharpP_Hardness", "anccft32.tex",           [dict(file="bio19_verify.py", ref="bio19_output.txt", quick=[], full=[], timeout=H)]),
